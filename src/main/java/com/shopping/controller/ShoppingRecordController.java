...
public class ShoppingRecordController {
    ...
    public Map<String,Object> addShoppingRecord(int userId,int productId,int counts){
        ...
        System.out.println("获得234"+product);
        if(counts<=product.getCounts()) {
            ...
            result = "success";
            System.out.println("添加成功123");
            ShoppingSale shoppingSale = shoppingSaleService.getSaleById(productId);
            System.out.println("567");
            if(shoppingSale ==null){
                System.out.println("789");
                ShoppingSale shoppingSale1=new ShoppingSale();
                shoppingSale1.setProductId(productId);
                System.out.println("454564");
                shoppingSale1.setCounts(counts);
                shoppingSale1.setTotalPrice(product.getPrice()*counts);
                shoppingSaleService.addSale(shoppingSale1);
                System.out.println("7875456");
            }else{
                System.out.println("8910");
               shoppingSale.setCounts(shoppingSale.getCounts()+counts);
               shoppingSale.setTotalPrice(shoppingSale.getTotalPrice()+product.getPrice() * counts);
               shoppingSaleService.updateSale(shoppingSale);
            }
        }
        else{
            System.out.println("111215");
            result = "unEnough";
        }
